//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace RainbowWine.Data
{
    using System;
    
    public partial class OrderDetail_Handover_BackToStore_Sel_New_Result
    {
        public int DeliveryBackToStoreId { get; set; }
        public Nullable<System.DateTime> CreatedDate { get; set; }
        public Nullable<int> OrderId { get; set; }
        public Nullable<int> ShopId { get; set; }
        public Nullable<int> DeliveryAgentId { get; set; }
        public string JobId { get; set; }
        public Nullable<double> OrderAmount { get; set; }
        public string Reason { get; set; }
        public Nullable<bool> ShopAcknowledgement { get; set; }
        public Nullable<System.DateTime> CollectedDate { get; set; }
        public int Id { get; set; }
        public System.DateTime OrderDate { get; set; }
        public decimal OrderAmount1 { get; set; }
        public Nullable<System.DateTime> DeliveryDate { get; set; }
        public string DeliveryPickup { get; set; }
        public int ODetail { get; set; }
        public int Id1 { get; set; }
        public int ProductID { get; set; }
        public int ItemQty { get; set; }
        public decimal Price { get; set; }
        public int pid { get; set; }
        public string ProductName { get; set; }
        public int ProductSizeID { get; set; }
        public string Capacity { get; set; }
        public Nullable<int> MixerOrderItemId { get; set; }
        public Nullable<int> MixerDetailId { get; set; }
        public Nullable<double> MixerPrice { get; set; }
        public Nullable<int> MixerItemQty { get; set; }
        public string MixerName { get; set; }
        public string MixerCapacity { get; set; }
    }
}
