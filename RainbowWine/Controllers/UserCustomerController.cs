...
namespace RainbowWine.Controllers
{
    ...
    [EnableCors("*", "*", "*")]
    public class UserCustomerController : ApiController
    {
        ...
        private decimal OrderAmtCheck(IList<APIOrderDetails> items)
        {
            decimal totalAmt = 0;
            ...
            using (var db = new rainbowwineEntities())
            {
                ...
                {
                    SpiritUtility.AppLogging($"Api_OrderAmtCheck: {ex.Message}", ex.StackTrace);
                }
                finally
                {
                    db.Dispose();
        ...
        [Route("add-order")]
        [Authorize(Roles = "Customer")]
        public HttpResponseMessage AddOrderByLoginUser(APIOrder model)
        {
            if (model == null)
            ...
            var amtTotal = OrderAmtCheck(model.OrderItems);
            if (amtTotal < intconfigAmtTotal)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, new ResponseStatus { Status = false, Message = $"Minimum order amount is {intconfigAmtTotal} rs." });
            }
        ...
        //{
        //    if (model == null)
        //        return Request.CreateResponse(HttpStatusCode.InternalServerError, new ResponseStatus { Status = false, Message = "Error Model Validation Failed.", Data = model });
        //    else if (model.AddressId <= 0)
        //        return Request.CreateResponse(HttpStatusCode.InternalServerError, new ResponseStatus { Status = false, Message = "Error Model Validation Failed. Address can not be null.", Data = model });
        //    else if (model.OrderItems == null || model.OrderItems.Count <= 0)
        //        return Request.CreateResponse(HttpStatusCode.InternalServerError, new ResponseStatus { Status = false, Message = "Error Model Validation Failed. Order Item can not be null", Data = model });
        ...
        //        if (!operationFlag)
        //            return Request.CreateResponse(HttpStatusCode.InternalServerError, new ResponseStatus { Status = false, Message = "Selected shop does not operate.", Data = model });
        ...
        //                //{
        //                //    CreateDatetime = DateTime.Now,
        //                //    Error = $"Api_add-order_SMS-Payment: {ex.Message}",
        //                //    Message = ex.StackTrace,
        //                //    MachineName = System.Environment.MachineName
        //                //});
        //                //db.SaveChanges();
        ...
        //            responseStatus.Status = true;
        //            responseStatus.Message = "Order created successfully.";
        //        }
        //        else
        //        {
        //            responseStatus.Status = false;
        //            responseStatus.Message = "Unable to create the order, order is null.";
        //        }
        //    }
        ...
        public HttpResponseMessage OrderTrack(int orderId)
        {
            ...
            using (var db = new rainbowwineEntities())
            {
                ...
                if (routeOrder != null)
                {
                    ...
                    var routePlans = routePlanDBO.DevlieryOrdersTrack(routeOrder.DeliveryAgentId ?? 0);
        ...
        public HttpResponseMessage OrderPaymentCashFreeLog(CashFreePayment cashFreePayment)
        {
            ...
            using (var db = new rainbowwineEntities())
            {
                try
                {
                    ...
                    int orderIdDecode = Convert.ToInt32(cashFreePaymentResponse.orderId);
        ...
        public HttpResponseMessage PaymentGetToken(CashFreeInput cashFreeInput)
        {
            ...
            try
            {
                ...
                var order = db.Orders.Find(Convert.ToInt32(cashFreeInput.OrderNo));
                if (order == null)
                {
                    responseStatus.Status = false;
        ...
        [HttpPost]
        [Route("OLD/delivery/orders")]
        [Authorize(Roles = "Deliver")]
        public HttpResponseMessage DeliveryOrdersOLD()
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = true };
            using (var db = new rainbowwineEntities())
            {
                ...
                db.Configuration.ProxyCreationEnabled = false;
                RoutePlanDBO routePlanDBO = new RoutePlanDBO();
                var routePlans = routePlanDBO.DevlieryOrders(regUser.DeliveryAgentId ?? 0, 6);
        ...
        [HttpPost]
        [Route("New/delivery/orders")]
        [Authorize(Roles = "Deliver")]
        public HttpResponseMessage DeliveryOrders6090()
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = true };
            using (var db = new rainbowwineEntities())
            {
        ...
        [HttpPost]
        [Route("v3.1.1/delivery/logon/{onoff}")]
        [Authorize(Roles = "Deliver")]
        public HttpResponseMessage OnDelivery(int onoff)
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = false };
            using (var db = new rainbowwineEntities())
            {
        ...
        [HttpGet]
        [Route("v3.1.1/delivery/logon-details")]
        [Authorize(Roles = "Deliver")]
        public HttpResponseMessage GetLoginDeliveryDetails()
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = false };
            using (var db = new rainbowwineEntities())
            {
        ...
        [HttpPost]
        [Route("OLD/delivery/orders/start/{jobid}")]
        [Authorize(Roles = "Deliver")]
        public HttpResponseMessage DeliveryOrdersStartOLD(string jobid)
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = true };
            using (var db = new rainbowwineEntities())
            {
                try
                {
                    ...
                    var route = db.RoutePlans.Where(o => string.Compare(o.JobId, jobid, true) == 0 && o.Order.OrderStatusId == 6).ToList();
                    if (route == null || route.Count() <= 0)
                    {
                        responseStatus.Message = "Currently there is no orders for delivery.";
                    }
        ...
        public HttpResponseMessage DeliveryOrdersStart(string jobid)
        {
            ...
            using (var db = new rainbowwineEntities())
            {
                try
                {
                    ...
                    RUser regUser = db.RUsers.Where(o => o.rUserId == uId)?.FirstOrDefault();
                    //db.Configuration.ProxyCreationEnabled = false;
                    int deliveryAgentId = regUser.DeliveryAgentId??0;


                    RoutePlanDBO routePlanDBO = new RoutePlanDBO();
                    //var route = routePlanDBO.DeliveryStart(jobid);
                    var route = routePlanDBO.DeliveryStart(jobid,deliveryAgentId);
        ...
        public HttpResponseMessage DeliveryOrderCompleted()
        {
            ...
            using (var db = new rainbowwineEntities())
            {
                ...
                RoutePlanDBO routePlanDBO = new RoutePlanDBO();
                var routePlans = routePlanDBO.DevlieredOrders(regUser.DeliveryAgentId ?? 0);
