...
namespace RainbowWine.Controllers
{
    ...
    public class UsersController : ApiController
    {
        ...
        [AllowAnonymous]
        [Route("temp/{id}")]
        public IHttpActionResult GetTempSms(string id)
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = true };

            //Flow SMS
            var dicti = new Dictionary<string, string>();
            dicti.Add("ORDERID", id);

            var templeteid = ConfigurationManager.AppSettings["Msg91TempOrderDelivered"];

            Task.Run(async()=>await Msg91Service.SendFlowSms(templeteid, "9881634394", dicti));  
            //
            return Ok("Success");
        }
        ...
        [AllowAnonymous]
        [Route("v2/login")]
        public HttpResponseMessage LoginV2(LoginViewModel model)
        {
            //throw new Exception("Applciation error");
            HttpResponseMessage response = null;
            var user = AuthenticateAsync(model);
            ...
            if (user != null)
            {
                ...
                using (var db = new rainbowwineEntities())
                {
                    ...
                    var otp = db.CustomerOTPVerifies.Where(o => o.CustomerId == uId);
                    if (otp.Count() > 0)
                    {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    using (var db = new rainbowwineEntities())
                    {
                        ...
                        if (custExists != null)
                        {
                            return Request.CreateResponse(HttpStatusCode.InternalServerError, new ResponseStatus { Status = false, Message = "This phone number already exists. Please try logging in with another phone number." });
                        }
                        db.Configuration.ProxyCreationEnabled = false;
                        ...
                        if (result.Succeeded)
                        {
                            ...
                            //response = Request.CreateResponse(HttpStatusCode.OK, new ResponseStatus { Status = true, Message = "Customer created successfully." });
                        }
                        else
                        {
        ...
        [Route("generateotp")]
        [Authorize(Roles = "Customer")]
        public HttpResponseMessage UserGenerateToken()
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = true };
        ...
        //            string textmsg = string.Format(ConfigurationManager.AppSettings["SMSGenToken"], token);
        //            wsms.SendMessage(textmsg, rcust.ContactNo);
        //            responseStatus.Message = "Verification token has send.";
        ...
        [HttpPost]
        [Route("verifyotp/{code}")]
        [Authorize(Roles = "Customer")]
        public HttpResponseMessage UserGenerateTokenVerify(string code)
        {
            ResponseStatus responseStatus = new ResponseStatus { Status = true };
            ...
            using (var db = new rainbowwineEntities())
            {
                try
                {
                    ...
                    {
                        if (string.Compare(code, otp.OTP) == 0)
                        {
